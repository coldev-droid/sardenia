...
# Constructing the 217 prose paragraphs for B02_C01.md
paras_raw = [
    # 1-10
    "The morning arrived with the heavy, persistent rhythm of autumn rain drumming steadily against the weathered terracotta roof tiles of the licensed guesthouse in Oristano, filling the quiet rooms with a deep, soothing murmur.",
...
# We can pad sentences with pure sensory details.
# Let's write a loop to measure words and expand naturally.

print(f"Initial paras count: {len(paras_raw)}")
initial_words = sum(len(p.split()) for p in paras_raw)
